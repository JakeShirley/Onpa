...
The Species feature loads `/api/v2/analytics/species/summary` for the full list of species the station has ever detected, with overall stats (count, first/last heard, max/avg confidence). It enriches each row with `/api/v2/species` (catalog metadata such as rarity and thumbnail) and reconciles very-recent activity from `/api/v2/detections/recent` so newly-heard species surface immediately even before the analytics roll-up catches up. The summary response decoder uses BirdNET-Go's snake_case field shape; catalog/detection responses keep the flexible decoder so they tolerate API shape changes. The merged list is cached per station for stale offline fallback. Species rows open detail pages that request `/api/v2/detections?queryType=species&species=<name>&numResults=<n>` for recent species-specific detections, use `/api/v2/media/species-image?name=<scientific-name>` for the hero image, and expose playable audio samples through `/api/v2/audio/:id`.
