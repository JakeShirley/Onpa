...
| FND-008 | done | Localization foundation | Localizable.xcstrings String Catalog with English (development), Spanish, German, and Japanese translations covering both static UI copy and dynamic strings (view-model status messages, AppError descriptions and recovery suggestions, accessibility labels, time-of-day and weather labels, station status/TLS displayName, percent/duration substitutions). knownRegions includes en/es/de/ja and AGENTS.md captures the ongoing policy. |
